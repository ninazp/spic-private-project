		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/fea/othercostinfo/feaOthercostinfoVO/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="feaOthercostinfoVO" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>	
		<table class="table table-bordered">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right">地热项目：</label></td>
					<td class="width-35">
					...
					<td class="width-15 active"><label class="pull-right">收入单位：</label></td>
					<td class="width-35">
				...
				<tr>
					...
					<td class="width-35">
						<form:input path="year1" htmlEscape="false"  min="0" class="form-control  isFloatGteZero"/>
					</td>
					...
					jp.get("${ctx}/fea/project/feaProjectB/getProjectById?id=" + projectId, function (data) {
						if(data.success){
						    ...
						    if(isNull(countyears) && isNull(len)){
						    	var startHidecol = countyears*2 + 4;  //8：前四个固定字段 *2（每隔字段两个td）
