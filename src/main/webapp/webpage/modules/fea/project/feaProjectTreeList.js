		...
		$(document).ready(function() {
			...
			$('#feaProjectjsTree').jstree({
				'core' : {
					...
					'data' : {
						"url" : "${ctx}/fea/project/feaProject/treeData",
						"dataType" : "json" 
					}
				},
					...
					"items": function (node) {
						...
						tmp.create = {
							...
							"action": function (data) {
								...
								jp.openDialog('添加下级项目登记', '${ctx}/fea/project/feaProject/form?parent.id=' + obj.id + "&parent.name=" + obj.text, '800px', '500px', $('#feaProjectjsTree'));
							}
						};
						tmp.remove = {
							"label": "删除项目登记",
							"action": function (data) {
								var inst = jQuery.jstree.reference(data.reference),
									obj = inst.get_node(data.reference);
								jp.confirm('确认要删除项目登记吗？', function(){
									jp.loading();
									$.get("${ctx}/fea/project/feaProject/delete?id="+obj.id, function(data){
										if(data.success){
											$('#feaProjectjsTree').jstree("refresh");
											jp.success(data.msg);
										}else{
											jp.error(data.msg);
										}
						...
						tmp.ccp = {
							...
							"action": function (data) {
								...
								jp.openDialog('编辑项目登记', '${ctx}/fea/project/feaProject/form?id=' + obj.id, '800px', '500px', $('#feaProjectjsTree'));
							}
						}
						return tmp;
					}
